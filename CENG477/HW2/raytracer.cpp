...
{   
    ...
    scene.takeTextureArrays();

    scene.EachSphereDeservesUniqueCenter();
    ...
    for(int c=0; c<scene.cameras.size(); c++){
        ...
        Vec3f su,sv,s,u;
        camera.gaze = normalize(camera.gaze);
        ...
        for (int y = 0; y < height; ++y) 
        {
            for (int x = 0; x < width; ++x)
            {
                ...
                double BETA = -1;
                double GAMMA = -1;
                rayHitsWhat(ray, which, itsindex, moreindex, t_min, true, BETA, GAMMA, scene);
                ...
                int TT[3];
                int* clr = TT;
                calculateColorOfPoint(scene.cameras[c].position,(ray.a + (ray.b - ray.a)*(t_min)), clr, which, itsindex, moreindex, BETA, GAMMA,scene);
        ...
        if(rec_dep){
            for (int y = 0; y < height; ++y)
            {
                for (int x = 0; x < width; ++x)
                {
                    ...
                    double BETA = -1;
                    double GAMMA = -1;
                    reflect(ray, mrr, rec_dep, rec_dep, BETA, GAMMA,scene);
    ...
    std::cout << "Total : " << elapsed_secs/60 << " min\a\n";
