...
namespace parser
{
    ...
    struct Texture
    {
        ...
        unsigned char* imgarray; //  = new unsigned char [w * h * 3];
        int width;
        int height;
    };
    ...
    struct Scene
    {
        ...
        void takeTextureArrays(){
            for(int i=0; i<textures.size(); i++){
                int w, h;
                char* iNAME = &textures[i].imageName[0];
                read_jpeg_header(iNAME, w, h);
                textures[i].imgarray = new unsigned char [w * h * 3];
                read_jpeg(iNAME, textures[i].imgarray, w, h);
                textures[i].width = w;
                textures[i].height = h;
            }
        }

        void EachSphereDeservesUniqueCenter(){
            for(int i=0; i<spheres.size(); i++){
                spheres[i].center = vertex_data[spheres[i].center_vertex_id-1];
            }
        }
